...
namespace VLB
{
    public class ShaderGenerator : ScriptableObject
    {
        ...
        public static Shader Generate(RenderPipeline rp, RenderingMode rm)
        {
            ...
            if (Instance == null)
            {
                ms_GenerationParamOnEnable = new GenerationParam { renderPipeline = rp, renderingMode = rm };
                return null;
            }

            return new GenShader(rp, rm).Generate();
        }
        ...
        public class GenPass
        {
            ...
            public string Generate(SRPHelper.RenderPipeline rp, RenderingMode rm, int passID, int passCount)
            {
        ...
        class GenShader
        {
            ...
            public GenShader(RenderPipeline rp, RenderingMode rm)
            {
                m_RenderingMode = rm;
                ...
                switch (rp)
                {
                    ...
                    case RenderPipeline.URP_or_LWRP:
                        AddPass(CullMode.Front);
            ...
            public Shader Generate()
            {
                ...
                for (int i = 0; i < m_Passes.Count; ++i)
                    passes += m_Passes[i].Generate(m_RenderPipeline, m_RenderingMode, i, m_Passes.Count);
        ...
        void OnEnable()
        {
            ...
            {
                Generate(ms_GenerationParamOnEnable.renderPipeline, ms_GenerationParamOnEnable.renderingMode);
